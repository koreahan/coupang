...
// 스크래핑 로직 개선: 성공률이 높은 프리미엄 프록시 렌더링을 우선적으로 시도
const sleep = (ms) => new Promise(r => setTimeout(r, ms));

async function getFastestHtml(finalUrl) {
  const ladder = [
    { render: true, timeout: 15000, premium: true }, // 프리미엄 프록시 우선 시도 (시간 넉넉하게)
    { render: true, timeout: 15000, premium: false }, // 일반 프록시 시도
    { render: false, timeout: 8000, premium: false }, // 렌더링 없이 빠른 시도
  ];
    ...
    for (let i = 0; i < 2; i++) {
      try {
        ...
        if (msg.includes('HTTP_429')) {
          ...
          continue; // 429 에러 시 잠시 대기 후 재시도
        }
        break; // 다른 에러 시 다음 단계로 넘어감
      }
    }
  }
...
// 파싱 로직 개선: JSON-LD와 메타 태그를 먼저 확인하고, 없으면 정규식으로 가격을 찾도록 순서를 최적화
function parseInfo(html){
  ...
  // 4순위: HTML 내 가격 후보 탐색 (JSON-LD와 og:title이 없을 때만 시도)
  const pricePatterns = [
    /data-rt-price="([\d,.]+)"/gi,
    /data-price="([\d,.]+)"/gi,
...
exports.handler = async (event) => {
  try {
    ...
    const { url, productInfo } = JSON.parse(event.body || "{}");
    ...
    return json({
      success: true,
      finalUrl,
      title: parsed.title || productInfo?.name || null,
      price: parsed.price || productInfo?.price || null,
